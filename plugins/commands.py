...
from database.tvseriesfilters import (
    add_tvseries_filter,
    update_tvseries_filter,
    getlinks,
    find_tvseries_filter,
    remove_tvseries,
    find_tvseries_by_first,
)
...
@Client.on_message(
    (filters.command("start") | filters.regex("Start")) & filters.incoming
)
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        btn = [
            [InlineKeyboardButton("🤖 Join Updates Channel", url="https://t.me/TMWAD")]
        ]
            ...
            try:
                ...
                btn.append(
                    [
                        InlineKeyboardButton(
                            " 🔄 Try Again",
                            url=f"https://t.me/{temp.U_NAME}/{message.command[1]}",
                        )
                    ]
                )
            except:
...
@Client.on_message(
    filters.command("addseries") & filters.incoming & filters.user(ADMINS)
)
async def tvseries_adder(bot, message):
    ...
    try:
        ...
        return await message.reply(
            "May Be Error is you puts space between links: \nUse correct format.<code>/addseries (name of series without space) (language eng/hindi/tamil/span) (quility 480/ 720/ 1080) (tv series batch links without space , use commas)</code>\n\n\nExample <code>/addseries strangerthings eng 480 https://tinyurl.com/23smxlh3,https://tinyurl.com/2yq2ghfh,https://tinyurl.com/27d9xyww,https://tinyurl.com/259az578</code>."
        )
    await sts.delete()


@Client.on_message(
    filters.command("updateseries") & filters.incoming & filters.user(ADMINS)
)
async def tvseries_updater(bot, message):
    sts = await message.reply("Checking Your Request...")
    if " " not in message.text:
        return await message.reply(
            "Use correct format.<code>/updateseries (name of series without space) (language eng/hindi/tamil/span) (quility 480/ 720/ 1080) (tv series batch links without space , use commas)</code>\n\n\nExample <code>/addseries strangerthings eng 480 https://tinyurl.com/23smxlh3,https://tinyurl.com/2yq2ghfh,https://tinyurl.com/27d9xyww,https://tinyurl.com/259az578</code>."
        )
    data = message.text.strip().split(" ")
    try:
        cmd, name, lang, quty, links = data
...
async def add_template(bot, message):
    ...
    if " " not in message.text:
        return await message.reply(
            "Use correct format. <code>/template (group id) (custom template)</code>"
        )
...
async def template_remover(bot, message):
    ...
    if " " not in message.text:
        return await message.reply(
            "Use correct format.<code>/removetemplate (group id)"
        )
    data = message.text.strip().split(" ")
    try:
        cmd, name = data
...
@Client.on_message(
    filters.command("removeseries") & filters.incoming & filters.user(ADMINS)
)
async def tvseries_remover(bot, message):
    sts = await message.reply("Checking Your Request...")
    if " " not in message.text:
        return await message.reply(
            "Use correct format.<code>/removeseries (name of series without space)"
        )
    data = message.text.strip().split(" ")
    try:
        cmd, name = data
...
@Client.on_message(
    (filters.command("notification") | filters.regex("Notification")) & filters.incoming
)
async def get_notification(bot, message):
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="On 🔛", callback_data="notification_on"),
                    InlineKeyboardButton(
...
async def notification_on(bot, query):
    ...
    return await query.message.edit(
        "Succesfully Turned on notifications 💌. use /notification to change"
    )
...
async def notification_off(bot, query):
    ...
    return await query.message.edit(
        "Succesfully Turned off notifications 💌. use /notification to change"
    )
...
async def A2Z_tvseries(bot, update):
    ...
    buttonz = ReplyKeyboardMarkup(split_list(Tvserieslist, 3), resize_keyboard=True)
...
@Client.on_message(
    (filters.command("tvseries") | filters.regex("Tv▫Series🔷") | filters.regex("Back↩"))
    & filters.private
)
...
async def homeseries(bot, update):
    buttonz = ReplyKeyboardMarkup(
        [["Tv▫Series🔷"], ["Start", "Notification"]], resize_keyboard=True
    )
    await bot.send_message(
        chat_id=update.chat.id,
        text="Selecet First leter of tv series",
